...
// STU: char name[16] + char note[16] + char stuid[8] + unsigned long age
void view(STU *s) {
    int id;
    printf("Please input id: ");
    scanf("%d",&id); // input here
    if(id < MAX) {
        printf("Name: %s\n", s[id].name);
        printf("Note: %s\n", s[id].stuid);
        printf("Age: %lu\n", s[id].age);
...
void edit_note(STU *s,SEC secret) {
	...
	if(check == secret.secret) {
		...
		if(id < MAX) {
            ...
            scanf("%d",&len);
            if(len < 16) {
                read(0,s[id].note,len); // input here XD
                puts("Done!");
            }
            else {
                puts("HACKER DETECTED!!");
...
void func() {
    STU S[MAX];   //MAX = 2  // STU: char name[16] + char note[16] + char stuid[8] + unsigned long age
    SEC secret;   // SEC: char info[32] + unsigned long secret
...
int main(int argc,char** argv,char** env) {
	setvbuf(stdin,0,2,0); // set to no buffer
	setvbuf(stdout,0,2,0); // set to no buffer
	setvbuf(stderr,0,2,0); // set to no buffer
    signal(SIGALRM,handler); 
    alarm(TIMEOUT); // TIMEOUT = 60

    delete_env(env); // clear all env?
    setenv("PATH",".",1);
